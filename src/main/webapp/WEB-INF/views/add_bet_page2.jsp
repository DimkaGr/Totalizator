            ...
            <div class="list-group" id="selectCompetition">
                <c:if test="${requestScope.competitions == null}">
                    <div class="text-center mb-4 p-3 alert alert-warning" role="alert">
                        ...
                        <h6><fmt:message key="message.bet.noCompetitions.addition" bundle="${bd}"/></h6>
                    </div>
                </c:if>
                <c:if test="${requestScope.competitions != null}">
                    <c:forEach var="competition" items="${requestScope.competitions}">
                        <a href="${pageContext.request.contextPath}/bets?command=add_bet3&id=${competition.id}&competitionName=${competition.participant1}-${competition.participant2}" class="list-group-item list-group-item-action"><c:out value="${competition.participant1}"/>-<c:out value="${competition.participant2}"/></a>
                    </c:forEach>
                </c:if>
            </div>
        </div>
